...
const useMainController = () => {
  ...
  // Initialize phone number with the required prefix
  const [phoneNumber, setPhoneNumber] = useState<string>("+85620");
  ...
  // Updated Validate form fields before submission
  const validateForm = (): boolean => {
    ...
    // Updated phone number validation to check for +85620 followed by 8 digits
    if (!phoneNumber?.trim()) {
      newErrors.phoneNumber = "ກະລຸນາປ້ອນເບີໂທລະສັບ";
    } else if (!/^\+85620\d{8}$/.test(phoneNumber.replace(/\s/g, ""))) {
      newErrors.phoneNumber = "ເບີໂທລະສັບຕ້ອງເປັນ +85620 ຕາມດ້ວຍຕົວເລກ 8 ຕົວ";
    }
  ...
  const resetForm = (): void => {
    ...
    setVillage("");
    setCity("");
    // Reset phone number to include the prefix
    setPhoneNumber("+85620");
  ...
  // Updated phone number change handler to enforce +85620 prefix
  const handlePhoneNumberChange = (
    ...
    // If input starts with +856 but not +85620, add the 20
    if (input.startsWith("+856") && !input.startsWith("+85620")) {
      const digits = input.substring(4).replace(/\D/g, "");
      const newValue = "+85620" + digits.substring(0, 8);
